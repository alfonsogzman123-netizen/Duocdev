...
class MaterialService {
  ...
  List<AcademicMaterial> get cachedMaterials => localCacheService.getCachedMaterials();

  Future<List<AcademicMaterial>> getMaterials() async {
    try {
      final data = await apiService.get('/materials') as List<dynamic>;
      final materials = data.map((item) => _fromJson(item as Map<String, dynamic>)).toList();
  ...
  Future<AcademicMaterial?> getMaterialById(String id) async {
    try {
      ...
      return localCacheService.getCachedMaterials().where((m) => m.id == id).firstOrNull;
    }
  }
  ...
  Future<void> saveMaterial(AcademicMaterial material) async {
    try {
      ...
      lastInfoMessage = 'Backend no disponible. Material guardado localmente y pendiente de sincronización.';
    }
  }

  AcademicMaterial _fromJson(Map<String, dynamic> json) => AcademicMaterial(
        ...
        summary: ((json['rawText'] as String?) ?? '').substring(0, (((json['rawText'] as String?) ?? '').length.clamp(0, 100))),
